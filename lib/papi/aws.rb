...
module Papi
  module AWS
	...
	unless request.sign
	  Papi.dprintf( 'Warning! Failed to sign request. No OpenSSL support for SHA256 digest.' )
	end
	...
	url = ENDPOINT[request.locale].path + request.query
      ...
      Papi.dprintf( 'Fetching http://%s%s ...', conn.address, url )
	...
	response = conn.get( url, { 'user-agent' => user_agent } )
      ...
      #
	     ...
	     Timeout::Error	=> error
	Papi.dprintf( 'Connection to server lost: %s. Retrying...', error )
	...
	Papi.dprintf( 'Following HTTP %s to %s ...', response.code, url )
	...
	if encoding
	  query << '&%s=%s' %
	    [ k, Papi.url_encode( @@encodings[encoding].iconv( v.to_s ) ) ]
	else
	  query << '&%s=%s' % [ k, Papi.url_encode( v.to_s ) ]
	end
      end
    ...
    class AWSObject
      ...
      # Example:
      #
      #  File.open( 'aws.dat' ) { |f| Papi::AWS::AWSObject.load( f ) }
      #
	...
	begin
	  ...
	  m = ex.to_s.match( /Papi::AWS::AWSObject::([^ ]+)/ )
      ...
      # Example:
      #
      #  File.open( 'aws.yaml' ) { |f| Papi::AWS::AWSObject.yaml_load( f ) }
      #
	...
	iv
      ...
      #
      def kernel  # :nodoc: 
	# E.g. Papi::AWS::SellerListingLookup -> seller_listing_lookup
	#
	stub = Papi.uncamelise( @__op__.class.to_s.sub( /^.+::/, '' ) )
	...
	if node.instance_of?( REXML::Document )
	  ...
	  name = Papi.uncamelise( node.name )
	...
	unless Papi::AWS::Error.const_defined?( *cd_params )
	  Papi::AWS::Error.const_set( err_class, Class.new( AWSError ) )
	end

	# Generate and return a new exception from the relevant class.
	#
	Papi::AWS::Error.const_get( err_class ).new( err_msg )
      end
    ...
    classes.each do |cl|
      ...
      uncamelised_name = Papi.uncamelise( class_name )
