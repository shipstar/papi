...
module Papi
  ...
  module AWS
    ...
    module ShoppingCart
      ...
      # this exception.
      #
      class CartError < Papi::AWS::Error::AWSError; end

      class Cart < Papi::AWS::Search::Request
	...
	# Papi::AWS::Search::Request.new for details of the parameters.
	#
	# Example:
	#
	#  cart = Cart.new
	...
	def in_area?(area, item_id_type, item_id)
	  found = area.find do |item|
	    item.send( Papi.uncamelise( item_id_type.to_s ) ).to_s == item_id
	  end
