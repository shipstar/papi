...
module Papi
  ...
  module AWS
	...
	#
	class AccessKeyIdError < Papi::AWS::Error::AWSError; end

	# Exception class for bad locales.
	#
	class LocaleError < Papi::AWS::Error::AWSError; end
	...
	def initialize(key_id=nil, associate=nil, locale=nil, cache=nil,
	  ...
	  @cache      = unless cache == 'false' || cache == false
			  Papi::AWS::Cache.new( @config['cache_dir'] )
			else
			  nil
			end
	...
	def locale=(l)  # :nodoc:
	  ...
	  #
	  if @tag == Papi::AWS::DEF_ASSOC[old_locale]
	    @tag = Papi::AWS::DEF_ASSOC[@locale]
	  end
	  ...
	  #
	  unless Papi::AWS::ENDPOINT[@locale] ==
		 Papi::AWS::ENDPOINT[old_locale]
	    #connect( @locale )
	    @conn = nil
	  end
	end
	...
	def cache  # :nodoc:
	  if @cache == true
	    @cache = Papi::AWS::Cache.new( @config['cache_dir'] )
	  else
	    @cache
	  end
	end
	...
	def validate_locale(l)
	  unless Papi::AWS::ENDPOINT.has_key? l
	...
	def connect(locale)
	  if ENV.key? 'http_proxy'
	    ...
	    @conn = Net::HTTP::Proxy( uri.host, uri.port, proxy_user,
				      proxy_pass ).start(
					Papi::AWS::ENDPOINT[locale].host )
	  else
	    @conn = Net::HTTP::start( Papi::AWS::ENDPOINT[locale].host )
	  end
	end
	private :connect
	...
	def error_check(xml)
	  if ! xml.nil? && xml = xml.elements['Errors/Error']
	    raise Papi::AWS::Error.exception( xml )
	  end
	end
	private :error_check
	...
	def sign
	  ...
	  Papi.dprintf( 'Calculating SHA256 HMAC of "%s"...', sign_str )

	  hmac = OpenSSL::HMAC.digest( DIGEST,
				       @config['secret_key_id'],
				       sign_str )
	  Papi.dprintf( 'SHA256 HMAC is "%s"', hmac.inspect )

	  base64_hmac = [ hmac ].pack( 'm' ).chomp
	  Papi.dprintf( 'Base64-encoded HMAC is "%s".', base64_hmac )

	  signature = Papi.url_encode( base64_hmac )
	...
	def search(operation, response_group=nil, nr_pages=1)
	  ...
	  @query = Papi::AWS.assemble_query( parameters, @encoding )
	  page = Papi::AWS.get_page( self )
	  ...
	  if xml = doc.elements['Result']
	    raise Papi::AWS::Error::AWSError, xml.text
	  end
